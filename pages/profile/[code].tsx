...
export default function PublicProfilePage() {
  ...
  const downloadVCard = async (platform: "ios" | "android") => {
    ...
    const noteLines = [
      ...
      twitter ? `Twitter: ${normalizeUrl(twitter)}` : null,
      instagram ? `Instagram: ${normalizeUrl(instagram)}` : null,
    ]
      .filter(Boolean)
      .join("\n");
...
ORG:${escapeVCard(orgSafe)}
TITLE:${escapeVCard(title)}
URL:${escapeVCard(fullURL)}
        ...
        <div className="flex justify-center mb-4">
          {profile.photo ? (
            ...
            <img
              src={profile.photo}
              alt="Profile"
              className="w-24 h-24 rounded-full object-cover border border-blue-600"
            />
          ) : (
